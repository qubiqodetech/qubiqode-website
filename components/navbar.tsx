          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 flex-col gap-x-2 gap-y-1">
              {group.items.map((item, idx) => (
                ...
                >
                  <div className="bg-stone-100 p-2 rounded-xl text-stone-500 group-hover/link:text-orange-600 group-hover/link:bg-orange-50 transition-colors shrink-0">
                    <item.icon className="w-4 h-4" />
                  </div>
                  <div>
                    <div className="text-sm font-semibold text-stone-900 group-hover/link:text-orange-600 transition-colors">
                      {item.name}
                    </div>
            ...
            <div className="pt-2 pb-6 flex flex-col gap-4 pl-4 border-l-2 border-stone-200 ml-2">
              {group.items.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-stone-600 flex items-center gap-3 text-lg font-medium hover:text-orange-600 transition-colors"
...
export function Navbar() {
  ...
  return (
    <>
      <motion.div
        className="fixed top-0 left-0 right-0 h-1 bg-orange-600 origin-left z-[70]"
        style={{ scaleX }}
      />
        ...
        {mobileMenuOpen && (
          ...
          >
            ...
            >
              <Link
                href="/contact"
                className="w-full inline-flex justify-center items-center gap-2 rounded-xl bg-orange-600 px-6 py-4 text-sm font-bold text-white shadow-xl shadow-orange-900/10 hover:bg-orange-500 active:scale-95 transition-all duration-200 uppercase tracking-widest"
                onClick={() => setMobileMenuOpen(false)}
              >
                Get a Quote
